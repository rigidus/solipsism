...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%ctract-def
   ...
   (%abstract %contract %identifier |%{| |%}|
              #'(lambda (ab ctract id l-brak r-brak) `(:contract ,id)))
  ...
  ;; todo: inheritance with contract-body
  ;; todo: abstract empty inheritance contract
  ;; todo: abstract inheritance with contract-body
  ...
  (%inher-spec
   ...
   ;; (%identifier-path %call-arg-list
   ;;                   #'(lambda (ip al) `(:identifier-path ,ip :call-arg-list ,al)))
   )
  (%ident-path
   (%ident #'(lambda (id) `(:ident-last ,id)))
   (%ident |%.| %ident-path #'(lambda (id dot ip) `(:ident-head ,id :ident-rest ,ip)))
   )
  (%ident
   (%identifier #'(lambda (id) `(:identifier ,id)))
...
#+BEGIN_SRC solidity :tangle tests/test_ctract_def.sol :mkdirp yes :noweb yes
  ...
  abstract contract test_empty_ctract_def
  {
  }
...
Теперь можно сделать тест, содержащий функцию с одним
