...
#+BEGIN_SRC lisp
  ...
  ;; (test-file #P"./tests/test_fuccall.sol")
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%yul-stmnt
   (%yul-stmnt-contents #'(lambda (last) `(:yul-src (,last))))
   (%yul-stmnt-contents %yul-stmnt
                       #'(lambda (head rest)
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%yul-block
   ...
   ( %yul-stmnt-contents #'(lambda (y-stmnt)
                             `(:stmnt ,y-stmnt))))
#+END_SRC
...
***** START yul-variable-declaration
  ...
  (%yul-func-call
   ...
   (%yul-func-call-body
    ...
    (%yul-expr |%,| %yul-func-call-body
               #'(lambda (exp1 cm exp2)
                   `(:y-fcallb-head ,exp :y-fcallb-rest ,exp2))))
...
#+BEGIN_SRC lisp :noweb-ref lexer_tokens
  ...
  ("\\baddmod\\b" '%asm-command 'addmod)
  ("\\bmulmod\\b" '%asm-command 'mulmod)
  ("\\bsignextend\\b" '%asm-command 'signextend)
  ("\\bkeccak256\\b" '%asm-command 'keccak256)
  ...
  ("\\breturn\\b" '%asm-command 'return)
  ("\\bbyte\\b" '%asm-command 'byte)
  ("\\brevert\\b" '%asm-command 'revert)
...
#+BEGIN_SRC solidity :tangle tests/test_fuccall.sol :mkdirp yes :noweb yes
  contract test_empty_yul_block
  {
      function fn_yul_var_decl (uint p_one) returns (uint r_one) {
          assembly {
