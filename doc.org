...
#+BEGIN_SRC lisp
  ...
  (funcall #'(lambda (file)
               ...
               (let* ((clj-lex (sol-lexer (alexandria:read-file-into-string file))))
                 (print (parse-with-lexer clj-lex *sol-parser*))))
           #P"/home/rigidus/src/solipsism/tests/test_revert_stmnt.sol")
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%stmnt
   ...
   (%revert-stmnt #'(lambda (x) `(:emit-stmnt ,x)))
   (%revert-stmnt %stmnt #'(lambda (x y) `(:emit-stmnt ,x :next-stmnt ,y)))
   ;; (%assembly-stmnt)
   )
#+END_SRC
