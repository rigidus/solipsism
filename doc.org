...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  ...
  (%empty-str-lit
   (|%"| |%"| #'(lambda (x) `(empty-str-lit |%"|)))
   (|%'| |%'| #'(lambda (x) `(empty-str-lit |%"|)))
   )
  (%non-empty-str-lit
   (|%"| %double-quoted-printable |%"| #'(lambda (x) `(empty-str-lit |%"|)))
   (|%"| %escape-sequence |%"| #'(lambda (x) `(empty-str-lit |%"|)))
   (|%'| %single-quoted-printable |%'| #'(lambda (x) `(empty-str-lit |%"|)))
   (|%'| %escape-sequence |%"| #'(lambda (x) `(empty-str-lit |%"|)))
   )
#+END_SRC
