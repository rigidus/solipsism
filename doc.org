...
#+BEGIN_SRC solidity :tangle tests/test_fn_type_name.sol :mkdirp yes :noweb yes
  pragma solidity ^0.8.0;
  library Strings {
      function test(uint256 value) internal pure returns (string memory) {
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%inline-array-expr
   (|%[| %expr-list |%]| #'(lambda (lb ex rb) `(:inline-array-expr ,ex)))
   )
  (%expr-list
   (%expr #'(lambda (ex) `(:expr-last ,ex)))
   (%expr %expr-list #'(lambda (ex el) `(:expr-head ,ex :expr-rest ,el)))
   )
#+END_SRC
