...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  ...
  (%source-unit-contents
   ...
   (%library-definition  #'(lambda (x) `(:lib-def ,x)))
   )
#+END_SRC
...
#+BEGIN_SRC solidity :tangle tests/test_contract_definition.sol :mkdirp yes :noweb yes
  <<test_pragma>>
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%library-definition
   ...
   (%library %identifier |%{| %library-contents |%}|
             #'(lambda (library id l-brak body-elt r-brak)
                 `(:library ,id :contents ,body-elt))))
  (%library-contents
   (%ctract-body-elt #'(lambda (x) `(:ctract-body-elt-last ,x)))
   (%ctract-body-elt %library-body-elt-contents
                       #'(lambda (x) `(:ctract-body-elt-head ,x :ctract-body-elt-rest ,y)))
   )
#+END_SRC
...
#+BEGIN_SRC solidity :tangle tests/test_library_definition.sol :mkdirp yes :noweb yes
  <<test_pragma>>
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%ctract-body-elt
   (%func-definition #'(lambda (x) `(ctract-body-elt ,x)))
   )
#+END_SRC

#+BEGIN_SRC solidity :tangle tests/test_library_contents.sol :mkdirp yes :noweb yes
  <<test_pragma>>
  library test_library_definition
  {
...
#+BEGIN_SRC lisp :noweb-ref grammar_terminals
  %unsigned-integer-type %signed-integer-type %string
#+END_SRC
