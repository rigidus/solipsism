...
#+BEGIN_SRC lisp
  ...
  ;; (test-file #P"./tests/test_event.sol")
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%event-def
   (%event %identifier |%(| |%)| |%;| #'(lambda (a b c d e)
                                                     `(:ident ,b)))
   (%event %identifier |%(| |%)| %anonymous |%;| #'(lambda (a b c d e f)
                                                     `(:ident ,b)))
   (%event %identifier |%(| %event-parameter |%)| |%;| #'(lambda (a b c d e f)
                                                           `(:ident ,b :event-param, d)))
   (%event %identifier |%(| %event-parameter |%)| %anonymous |%;| #'(lambda (a b c d e f g)
                                                                      `(:ident ,b :event-param, d))))
  ...
  (%event-parameter
   (%type-name  #'(lambda (a)
                    `(:t-name ,a)))
   (%type-name |%,| %event-parameter #'(lambda (a b c)
                                         `(:t-name ,a :body-rest ,c)))

   (%type-name %indexed #'(lambda (a b)
                            `(:t-name ,a)))
   (%type-name %indexed |%,| %event-parameter #'(lambda (a b c d)
                                                   `(:t-name ,a :body-rest ,d)))


   (%type-name %identifier #'(lambda (a b)
                               `(:t-name ,a :ident ,b)))
   (%type-name %identifier |%,| %event-parameter #'(lambda (a b c d)
                                                     `(:t-name ,a :ident ,b :body-rest ,d)))


   (%type-name %indexed %identifier #'(lambda (a b c)
                                        `(:t-name ,a :ident ,c)))
   (%type-name %indexed %identifier |%,| %event-parameter #'(lambda (a b c d e)
                                                              `(:t-name ,a :ident ,c :body-rest ,e))))
...
#+BEGIN_SRC solidity :tangle tests/test_event.sol :mkdirp yes :noweb yes
  <<test_pragma>>
  contract test_event_definition
  {
      event some();
      event some() anonymous;

      event some(p_one);
      event some(p_one) anonymous;

      event some(p_one, p_two);
      event some(p_one, p_two) anonymous;
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%enum-def
   (%enum %identifier |%{| %enum-body |%}| #'(lambda (a b c d e)
                                                `(:ident ,b :enum-body, d))))

  (%enum-body
   ( %identifier #'(lambda (a)
                     `(:enum-body ,a)))
   ( %identifier |%,| %enum-body #'(lambda (a b c)
                                     `(:body-head ,a :body-rest ,c))))
#+END_SRC
