...
Лексер на вход получает поток символов - сам код, разбирает их на лексемы
(группы символов) на выходе возвращает токены.

Парсер берет эти токены и производит синтаксический анализ.

[[TODO:gmm]] приведи плиз пример лексемы и токена для солидити
...
[[TODO:gmm]] что такое плейсхолдер?
...
в грамматическое правило само это правило. [[TODO:gmm]] зачем?

Мы будем использовать его неоднократно далее. Source-unit содержит
source-unit-contents, который в свою очередь может рекурсивно содержать source-unit.

А source-unit-contents в свою очередь содержит все что есть в
контракте.

[[TODO:gmm]] что это за знак %? Все, что начинается с этого знака - это
терминал? В ANSI common lisp этого не было, гуглится
херово. И что за двоеточия перед src-last и т.д.?
...
[[TODO;gmm]] в смысле опережающее определение функции?
...
[[TODO:gmm]] Т.е. здесь ты определяешь все возможно допустимые типы, которые
юзаются в солидити?
...
[[TODO:gmm]] до сих пор не понятна разница между терминалом и нетерминалом, а
так же что это в принципе.
