...
#+BEGIN_SRC lisp
  ...
  (funcall #'(lambda (file)
               ...
               (let* ((clj-lex (sol-lexer (alexandria:read-file-into-string file))))
                 (print (parse-with-lexer clj-lex *sol-parser*))))
           #P"/home/rigidus/src/solipsism/tests/test_try_catch_stmnt.sol")
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%stmnt
   ...
   (%break-stmnt #'(lambda (x) `(:cont-stmnt ,x)))
   (%break-stmnt %stmnt #'(lambda (x y) `(:cont-stmnt ,x :next-stmnt ,y)))
   (%try-stmnt #'(lambda (x) `(:cont-stmnt ,x)))
   (%try-stmnt %stmnt #'(lambda (x y) `(:cont-stmnt ,x :next-stmnt ,y)))
   ;; (%return-stmnt)
   ;; (%emit-stmnt)
   ;; (%revert-stmnt)
   ;; (%assembly-stmnt)
...
*** START try-statement and catch-clause
