...
Процедура тестирования будет пробегаться по каждому из них и передавать в
парсер:
  ...
  (map-subdir-files
   ...
   #'(lambda (file)
       (let* ((clj-lex (sol-lexer (alexandria:read-file-into-string file)))
              (results (parse-with-lexer clj-lex *sol-parser*)))
         (print results))))
#+END_SRC
...
#+NAME: test_pragma
#+BEGIN_SRC solidity :tangle tests/pragma.sol :noweb tangle :padline no
  pragma solidity ^0.8.0;
#+END_SRC
