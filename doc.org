...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  ...
  (%err-param-contents
   (%err-param #'(lambda (id) `(:err-par-last ,id)))
   (%err-param |%,| %err-param-contents
               #'(lambda (ep cm epc)
                   `(:err-param-head ,ep :err-param-rest ,epc)))
   )
  (%err-param
...
#+BEGIN_SRC solidity :tangle tests/test_error_def.sol :mkdirp yes :noweb yes
  ...
  error MyError2 ( bytes , uint256 ) ;
  error MyError3 ( bytes arr ) ;
  error MyError4 ( bytes arr , uint256 someval ) ;
#+END_SRC
