...
#+BEGIN_SRC lisp
  ...
  (test-all)
  ;; (test-file #P"/home/rigidus/src/solipsism/tests/test_library_definition.sol")
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%ctract-def
   ...
   (%contract %identifier |%{| %ctract-def-contents |%}|
            #'(lambda (ctract id l-brak contents r-brak)
                `(:contract ,id :contents ,contents)))
   )
  (%ctract-contents
   (%func-def #'(lambda (fd) `(:func-last ,fd)))
   (%func-def %ctract-contents  #'(lambda (fd cc) `(:func-head ,fd :func-rest ,cc)))
   )
#+END_SRC
...
Определение библиотеки очень похоже на определение контракта.
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%lib-def
   ...
   (%library %identifier |%{| %lib-contents |%}|
             #'(lambda (library id l-brak body-elt r-brak)
                 `(:library ,id :contents ,body-elt))))
  (%lib-contents
   (%ctract-body-elt #'(lambda (x) `(:ctract-body-elt-last ,x)))
   (%ctract-body-elt %lib-contents
...
NB: Опережающий тест, использует конструкции, определенные в следующих
параграфах.
...
*** COMMENT Parlist и ParameterList
...
На этом этапе можно сделать тест, содержащий функцию с одним
параметром, который определен только как тип
...
#+BEGIN_SRC lisp :noweb-ref func_def_contents
  (%function %identifier %parlist %retlist |%;|
...
#+BEGIN_SRC lisp :noweb-ref func_def_contents
  (%function %identifier %parlist %fmeta %retlist |%;|
...
[TODO:gmm] %call-argument-list это что-то сложное, поэтому пока представим
его как %parlist
