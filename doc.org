...
#+BEGIN_SRC lisp :noweb-ref func_def_contents
  (%func %identifier %parlist %overblock
         #'(lambda (fun id parlist blk)
             `(:fun ,id :parlist ,parlist :overblock ,blk)))
#+END_SRC
...
#+BEGIN_SRC lisp :noweb-ref func_def_contents
  (%func %identifier %parlist %retlist %overblock
         #'(lambda (fun id parlist retlist blk)
             `(:fun ,id :parlist ,parlist :retlist ,retlist :overblock ,blk)))
#+END_SRC
...
#+BEGIN_SRC lisp :noweb-ref func_def_contents
  (%func %identifier %parlist %fmeta %retlist %overblock
...
Переходим к содержимому функции. Внутри функции может быть "overblock"
- название выбрано с учетом того что в грамматике используется
нетерминал "block", который встречается в других контекстах. Этот
"overblock" может быть:
- просто точкой с запятой
- нетерминалом "block"

#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%overblock
   (|%;| #'(lambda (sc) `(:semicolon ,sc)))
   (%block #'(lambda (blk) `(:block blk)))
   )
#+END_SRC
...
- любым количеством нетерминалов вида "unchecked-block", заключенных в
  фигурные скобки, поэтому тут требуется снова применить трюк с оборачивающим contents-правилом.

#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%block
   (|%{| |%}| #'(lambda (l-brak r-brak) `(:empty-block 'empty)))
   (|%{| %statement |%}| #'(lambda (l-brak stmnt r-brak) `(:block stmnt)))
   (|%{| %unchecked-block-contents |%}|
         #'(lambda (l-brak cont r-brak) `(:unchecked ,cont)))
   )
  (%unchecked-block-contents
   (%unchecked-block #'(lambda (x) `(:unchk-blk-last ,x)))
   (%unchecked-block %unchecked-block-contents
                     #'(lambda (a b) `(:unchk-blk-head ,a :unchk-blk-rest ,b)))
   )
#+END_SRC

В тесте мы проверим, что unchecked-block могут вкладываться друг в друга.

Наконец сам нетерминал unchecked-block может состоять только из терминала "unchecked" со следующим за ним нетерминалом "block":

#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%unchecked-block
   (%unchecked %block #'(lambda (a b) `(:unchecked-bloc ,b)))
   )
#+END_SRC
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%statement
   (%block #'(lambda (blk) `(:block blk)))
   (%var-decl-stmnt-contents #'(lambda (a) `(:vars a)))
   ;; (%expression-statement-contents)
   ;; (%if-statement)
   ;; (%for-statement)
   ;; (%while-statement)
   ;; (%do-while-statement)
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%var-decl-stmnt-contents
   ...
   (%var-decl-stmnt %var-decl-stmnt-contents
                    #'(lambda (a b)
                        `(:var-decl-stmnt-head ,a :var-decl-stmnt-rest ,b)))
   )
  (%var-decl-stmnt
   (%var-decl |%;| #'(lambda (vd sc) `(:var-decl ,vd)))
   ;; (%var-decl |%=| %expression |%;|)
   ;; (%var-decl-typle |%=| %expression |%;|)
   )
#+END_SRC

Собственно объявление переменной - это объявление ее type-name,
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%var-decl
   ...
   ;; (%type-name %data-location %identifier |%;|
   ;;             #'(lambda (a b c)
   ;;                 `(:var-type ,a :data-location ,b :name ,c)))
   )
#+END_SRC
