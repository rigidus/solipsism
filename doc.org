...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%src-unit
   ...
   (%src-unit-contents %src-unit
                       #'(lambda (head rest)
                           `(:src ,(append (list head) (getf rest :src)))))
   )
  (%src-unit-contents
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%ctract-body-elt-contents
   ...
   (%ctract-body-elt %ctract-body-elt-contents
                     #'(lambda (head rest)
                         `(:ctract-body-elt ,(append (list head)
                                                     (getf rest :ctract-body-elt)))))
   )
  (%ctract-body-elt
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  ...
  (%event-params-contents
   ...
   (%event-param |%,| %event-params-contents
                 #'(lambda (head co rest)
                     `(:event-param ,(append (list head)
                                             (getf rest :event-param)))))
   )
  (%event-param
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  ...
  (%struct-member-contents
   ...
   (%struct-member %struct-member-contents
                   #'(lambda (head rest)
                       `(:member ,(append (list head) (getf rest :memeber)))))
   )
  (%struct-member
   (%type-name %identifier |%;|
               #'(lambda (tn id sc)
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  ...
  (%err-param-contents
   ...
   (%err-param |%,| %err-param-contents
               #'(lambda (head cm rest)
                   `(:err-par ,(append (list head) (getf rest :err-par)))))
   )
  (%err-param
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  ...
  ;;                  `(:const-var-decl ,id :type-name ,tn :val ,expr)))
  ;;  )
#+END_SRC
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%stmnt
   ...
   (%var-decl-stmnt #'(lambda (last) `(:var-decl-stmnt ,x)))
   (%var-decl-stmnt %stmnt #'(lambda (head rest) `(:var-decl-stmnt ,x :next-stmnt ,y)))

   (%expr-stmnt #'(lambda (last) `(:expr-stmnt ,x)))
   (%expr-stmnt %stmnt #'(lambda (head rest) `(:expr-stmnt ,x :next-stmnt ,y)))

   (%if-stmnt #'(lambda (last) `(:if-stmnt ,x)))
   (%if-stmnt %stmnt #'(lambda (head rest) `(:if-stmnt ,x :next-stmnt ,y)))

   (%for-stmnt #'(lambda (last) `(:for-stmnt ,x)))
   (%for-stmnt %stmnt #'(lambda (head rest) `(:for-stmnt ,x :next-stmnt ,y)))

   (%while-stmnt #'(lambda (last) `(:while-stmnt ,x)))
   (%while-stmnt %stmnt #'(lambda (head rest) `(:while-stmnt ,x :next-stmnt ,y)))

   (%do-while-stmnt #'(lambda (last) `(:dowhile-stmnt ,x)))
   (%do-while-stmnt %stmnt #'(lambda (head rest) `(:dowhile-stmnt ,x :next-stmnt ,y)))

   (%cont-stmnt #'(lambda (last) `(:cont-stmnt ,x)))
   (%cont-stmnt %stmnt #'(lambda (head rest) `(:cont-stmnt ,x :next-stmnt ,y)))

   (%break-stmnt #'(lambda (last) `(:break-stmnt ,x)))
   (%break-stmnt %stmnt #'(lambda (head rest) `(:break-stmnt ,x :next-stmnt ,y)))

   (%try-stmnt #'(lambda (last) `(:try-stmnt ,x)))
   (%try-stmnt %stmnt #'(lambda (head rest) `(:try-stmnt ,x :next-stmnt ,y)))

   (%return-stmnt #'(lambda (last) `(:return-stmnt ,x)))
   (%return-stmnt %stmnt #'(lambda (head rest) `(:return-stmnt ,x :next-stmnt ,y)))

   (%emit-stmnt #'(lambda (last) `(:emit-stmnt ,x)))
   (%emit-stmnt %stmnt #'(lambda (head rest) `(:emit-stmnt ,x :next-stmnt ,y)))

   (%revert-stmnt #'(lambda (last) `(:revert-stmnt ,x)))
   (%revert-stmnt %stmnt #'(lambda (head rest) `(:revert-stmnt ,x :next-stmnt ,y)))

   (%assembly-stmnt #'(lambda (last) `(:assembly-stmnt ,x)))
   (%assembly-stmnt %stmnt #'(lambda (head rest) `(:assembly-stmnt ,x :next-stmnt ,y)))
   )
#+END_SRC
