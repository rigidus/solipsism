...
#+BEGIN_SRC lisp
  ...
  ;; (test-file #P"./tests/test_var_decl.sol")
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%yul-block
   ...
   ( %yul-stmnt #'(lambda (y-stmnt) `(:stmnt ,y-stmnt))))
#+END_SRC
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%yul-var-decl
   (%let %yul-identifier #'(lambda (var id)
                             `(:v-id ,id)))

   (%let %yul-identifier |%:=| %yul-expr
...
[[TODO:gmm]] заменить yul-identifier на identifier везде
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%yul-path
   (%yul-identifier #'(lambda (yid) `(:y-path-head ,yid)))
   (%yul-identifier |%.| %yul-path #'(lambda (yid1 pt yid2)
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%yul-func-call
   ...
   (%yul-identifier |%(| %yul-func-call-body |%)|
                    #'(lambda (yid lsc rsc) `(:y-id ,yid)))
