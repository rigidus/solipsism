...
%identifier-path может быть одним идентифактором илиз набором из идентификаторов,
разделенных точками:
...
#+BEGIN_SRC solidity :tangle tests/test_fn_mod_inv.sol :mkdirp yes :noweb yes
  ...
  contract test_fn_mod_inv
  {
      ...
      function fn_1 (uint p_one) OkObj.isOk() returns (uint r_one) ;
  }
#+END_SRC

[TODO:gmm] -  override-specifier тянет за собой identifier-path
