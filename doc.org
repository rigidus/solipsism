...
#+BEGIN_SRC lisp
  ...
  (funcall #'(lambda (file)
               ...
               (let* ((clj-lex (sol-lexer (alexandria:read-file-into-string file))))
                 (print (parse-with-lexer clj-lex *sol-parser*))))
           #P"/home/rigidus/src/solipsism/tests/test_break_stmnt.sol")
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%stmnt
   ...
   (%cont-stmnt #'(lambda (x y) `(:cont-stmnt ,x :next-stmnt ,y)))
   (%break-stmnt #'(lambda (x) `(:cont-stmnt ,x)))
   (%break-stmnt #'(lambda (x y) `(:cont-stmnt ,x :next-stmnt ,y)))
   ;; (%try-stmnt)
   ;; (%return-stmnt)
   ;; (%emit-stmnt)
   ;; (%revert-stmnt)
