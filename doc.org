...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  (%using-def
   (%using %ident-path %for |%*| |%;|
           #'(lambda (us id f s sc)
               `(:using-ident-path ,id :using-star ,s)))
   (%using %ident-path %for %type-name |%;|
           #'(lambda (us id f t-name sc)
               `(:using-ident-path ,id :using-t-name ,t-name))))
#+END_SRC
...
#+BEGIN_SRC lisp :noweb-ref grammar_rules
  ...
  (%event-params-contents
   (%event-param #'(lambda (ep) `(:event-param-last ,ep)))
   (%event-param |%,| %event-params-contents
                 #'(lambda (ep co epc)
                     `(:event-param-head ,ep ::event-param-rest ,epc)))
   )
  (%event-param
...
#+BEGIN_SRC solidity :tangle tests/test_event_def.sol :mkdirp yes :noweb yes
  contract test_event_def
  {
      ...
      event some(bytes);
      event some(bytes) anonymous;

      event some(uint, int);
      event some(uint, int) anonymous;

      event Deposit(address indexed from, bytes32 indexed id, uint val);
      event Deposit(address indexed from, bytes32 indexed id, uint val) anonymous;
  }
#+END_SRC
