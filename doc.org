...
Переходим к содержимому функции. Внутри функции может быть "блок",
который может быть просто точкой с запятой или какими-то выражениями
...
#+BEGIN_SRC solidity :tangle tests/test_fn_block.sol :mkdirp yes :noweb yes
  ...
  contract test_fn_override
  {
      function fn_block (uint p_one) returns (uint r_one) { }
  }
#+END_SRC
